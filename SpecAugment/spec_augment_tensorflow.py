...
def frequency_masking(mel_spectrogram, v, frequency_masking_para=27, frequency_mask_num=2):
    ...
    for i in range(frequency_mask_num):
        f = tf.random.uniform(
            [], minval=0, maxval=frequency_masking_para, dtype=tf.int32)
...
def time_masking(mel_spectrogram, tau, time_masking_para=100, time_mask_num=2):
    ...
    for i in range(time_mask_num):
        t = tf.random.uniform(
            [], minval=0, maxval=time_masking_para, dtype=tf.int32)
...
def spec_augment(mel_spectrogram):
    ...
    warped_frequency_time_sepctrogram = time_masking(
...
def visualization_spectrogram(mel_spectrogram, title):
    ...
    librosa.display.specshow(librosa.power_to_db(
        mel_spectrogram[0, :, :, 0], ref=np.max), y_axis='mel', fmax=8000, x_axis='time')
    plt.title(title)
    plt.tight_layout()
    plt.show()
